...
| T2 | **일별 스냅샷** | 매일 03:00 KST | **Worker `snapshotVideos`** 또는 REST `…/snapshots/now` 배치 → Snapshots DB upsert *(에이전트 스케줄러가 동일 도구를 결정적으로 호출)* | 노출확률 계산용 시계열 |
...
| `snapshotVideos` | Videos DB 행의 영상 목록을 읽고 일괄 스냅샷 → Snapshots DB | **일별 스케줄**에 매핑 (**Custom Agent Scheduler**가 동일 도구 재호출) |
