...
function Sidebar({
  ...
  // Use context state for SSR and first paint so server-supplied defaultOpen is visible immediately.
  const effectiveState = state;
  const effectiveCollapsible = state === 'collapsed' ? collapsible : '';
...
function SidebarHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      data-sidebar="header"
      className={cn(
        'flex flex-col gap-2 p-2 **:data-[sidebar=menu-button]:rounded-sm',
        className
      )}
      {...props}
    />
